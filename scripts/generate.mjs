...
const copy = {
  'zh-CN': {
    ...
    projectsKicker: '生态项目', projectsTitle: '一个框架，多个项目', projectsLead: '每个项目拥有独立入口与自己的多语言文档，同时共享一致、清晰的使用体验。',
  ...
  'zh-TW': {
    ...
    getStarted: '開始使用', download: '下載最新版本', downloadGithub: '從 GitHub 下載', viewSource: '查看原始碼', readDocs: '閱讀文件', githubUnavailable: 'GitHub 無法下載？', communityLabel: '加入社群', faqKicker: '常見問題', stars: 'GitHub Stars', release: '最新版本', updated: '程式更新', online: '開源可用',
    capabilities: '核心能力', capabilitiesTitle: '為實際自動化場景而生', capabilitiesLead: '完整涵蓋辨識、執行與日常使用。', projectsKicker: '專案生態', projectsTitle: '一個框架，多個專案', projectsLead: '每個專案都有獨立入口與多語言文件。',
  ...
  ja: {
    ...
    getStarted: 'はじめる', download: '最新版をダウンロード', downloadGithub: 'GitHub からダウンロード', viewSource: 'ソースを見る', readDocs: 'ドキュメント', githubUnavailable: 'GitHub から取得できない場合', communityLabel: 'コミュニティ', faqKicker: 'よくある質問', stars: 'GitHub Stars', release: '最新リリース', updated: '最終更新', online: 'オープンソース',
    capabilities: '主な機能', capabilitiesTitle: '実用的な自動化のために', capabilitiesLead: '認識から操作、日常の実行までを一つのツールで。', projectsKicker: 'エコシステム', projectsTitle: '一つの基盤、多彩なプロジェクト', projectsLead: 'プロジェクトごとに独立した入口と多言語ドキュメントを提供します。',
...
function landingPage(project, locale, meta, faq = null) {
  ...
  ].map(option => `<a class="dropdown-option download-option" role="menuitem" href="${escapeHtml(option.url)}"><span>${option.label}</span><strong>${escapeHtml(meta.release)}</strong></a>`).join('');
  const downloadControl = `<div class="custom-dropdown download-split" data-dropdown>
    <a class="button primary download-main" href="${escapeHtml(primaryUrl)}">${githubIcon(project, currentUrl)}<span>${t.downloadGithub}</span><strong>${escapeHtml(meta.release)}</strong><small>${defaultDownloadLabel}</small><span aria-hidden="true">↓</span></a>
